...
public class InstrutorController {
    ...
    public ResponseEntity<DadosDetalhamentoInstrutor> cadastroInstrutor(
    ...
    @GetMapping
    public ResponseEntity<Page<DadosDetalhamentoInstrutor>> listarInstrutores(
            @PageableDefault(size = 10, sort = {"nome"}) Pageable paginacao) {
        Page page = repository.findAllByAtivoTrue(paginacao).map(DadosDetalhamentoInstrutor::new);
    ...
    public ResponseEntity<DadosDetalhamentoInstrutor> atualizarInstrutor(
            @RequestBody @Valid DadosAtualizarInstrutor dados){
